...
# -----------------------------Add, Delete, Update and List Audio Files------------------------------------- #
    ...
    @admin_bp.route('/admin/audio-files/<audioFiles_id>', methods=['DELETE'])
    @jwt_required()
    def delete_audioFiles(audioFiles_id):
        existing_audioFiles = mongo.db.audioFiles.find_one(
            {'_id': ObjectId(audioFiles_id)})
        if not existing_audioFiles:
            return jsonify({'error': 'Audiofile not found'}), 404
        
        filename = extract_filename_from_url(existing_audioFiles['file_url'])
        file_path = os.path.join("files", "audioFiles", filename)
        if os.path.exists(file_path):
            os.remove(file_path)

        mongo.db.audioFiles.delete_one(
            {'_id': ObjectId(audioFiles_id)})
        return jsonify({'message': 'Audiofile deleted successfully'}), 200
